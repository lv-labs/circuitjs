...
public class UIManager {
	...
	static final int MENUBARHEIGHT = 38;
	static final int TOOLBARHEIGHT = 40;
	static int VERTICALPANELWIDTH = 220; // default
	long lastResizeTime;
	...
	void init() {
		...
		if (VERTICALPANELWIDTH > 240)
			VERTICALPANELWIDTH = 240;
		if (VERTICALPANELWIDTH < 196)
			VERTICALPANELWIDTH = 196;
		...
		buttonPanel.getElement().getStyle().setProperty("justifyContent", "center");
		buttonPanel.getElement().getStyle().setProperty("alignItems", "center");
		buttonPanel.getElement().getStyle().setProperty("gap", "8px");
		buttonPanel.getElement().getStyle().setProperty("width", "100%");
		...
		menus.toolbarCheckItem.setState(!hideMenu && !noEditing && !hideSidebar && app.startCircuit == null
				&& app.startCircuitText == null && app.startCircuitLink == null);
		...
		styleControlButton(resetButton, false);
		...
		runStopButton.addStyleName("run-stop-button");
		styleRunStopButton(false);
	...
	void syncSidebarBackground() {
		if (verticalPanel == null)
			return;
		com.google.gwt.dom.client.Element panelElement = verticalPanel.getElement();
		panelElement.getStyle().setBackgroundColor(Theme.uiSurface());
		com.google.gwt.dom.client.Element parent = panelElement.getParentElement();
		if (parent != null)
			parent.getStyle().setBackgroundColor(Theme.uiSurface());
		com.google.gwt.dom.client.Element grandParent = parent == null ? null : parent.getParentElement();
		if (grandParent != null)
			grandParent.getStyle().setBackgroundColor(Theme.uiSurface());
	}
	...
	void styleControlButton(Button button, boolean accent) {
		Style style = button.getElement().getStyle();
		style.setBackgroundColor(accent ? Theme.uiAccent() : Theme.uiControlBg());
		style.setProperty("background", accent ? Theme.uiAccent() : Theme.uiControlBg());
		style.setColor(Theme.uiText());
		style.setBorderWidth(1, Unit.PX);
		style.setBorderColor(accent ? Theme.uiAccent() : Theme.uiBorder());
		...
		style.setProperty("fontFamily", Theme.uiFont());
		style.setProperty("fontSize", Theme.uiFontSize());
		style.setProperty("fontWeight", Theme.uiFontWeight());
		style.setCursor(Style.Cursor.POINTER);
	}

	void styleRunStopButton(boolean running) {
		styleControlButton(runStopButton, !running);
		runStopButton.removeStyleName("run-stop-stopped");
		if (!running) {
			runStopButton.addStyleName("run-stop-stopped");
		}
	}
	...
	public void setSimRunning(boolean s) {
		if (s) {
			if (app.stopMessage != null)
				return;
			app.simRunning = true;
			runStopButton.setText(Locale.LS("Running"));
			styleRunStopButton(true);
			app.timer.scheduleRepeating(app.FASTTIMER);
		} else {
			app.simRunning = false;
			runStopButton.setText(Locale.LS("Stopped"));
			styleRunStopButton(false);
			app.timer.cancel();
			repaint();
		}
	...
	public void updateCircuit() {
		...
		if (menus.printableCheckItem.getState()) {
			...
			g.setColor(Theme.canvasBgColor());
			cv.getElement().getStyle().setBackgroundColor(Theme.canvasBg());
		}
	...
	void drawCanvasGrid(Graphics g, double scale) {
		int minX = mouse.inverseTransformX(0);
		int maxX = mouse.inverseTransformX(app.circuitArea.width);
		int minY = mouse.inverseTransformY(0);
		int maxY = mouse.inverseTransformY(app.circuitArea.height);

		int startX = app.snapGrid(minX);
		int endX = app.snapGrid(maxX + app.gridSize);
		int startY = app.snapGrid(minY);
		int endY = app.snapGrid(maxY + app.gridSize);

		g.context.save();
		g.context.setStrokeStyle(Theme.canvasGrid());
		g.context.setLineWidth(1.0 / Math.max(1, app.transform[0] * scale));

		for (int x = startX; x <= endX; x += app.gridSize) {
			g.context.beginPath();
			g.context.moveTo(x, minY);
			g.context.lineTo(x, maxY);
			g.context.stroke();
		}

		for (int y = startY; y <= endY; y += app.gridSize) {
			g.context.beginPath();
			g.context.moveTo(minX, y);
			g.context.lineTo(maxX, y);
			g.context.stroke();
		}
	...
	void drawBottomArea(Graphics g) {
		...
		if (app.stopMessage != null && app.circuitArea.height > canvasHeight - 30)
			h = 30;
		g.setColor(menus.printableCheckItem.getState() ? "#eee" : Theme.scopeBg());
		...
		if (app.stopMessage != null) {
			...
			if (mouse.getMouseElm() != null) {
				if (mouse.mousePost == -1) {
					...
					/*
					 * // show node numbers for debugging int ni; for (ni = 0; info[ni] != null &&
					 * ni < info.length-1; ni++) ; try { String nodeStr = "nodes:"; for (int nn = 0;
					 * nn < mouse.getMouseElm().getNodeCount(); nn++) nodeStr += " " +
					 * mouse.getMouseElm().getNode(nn).index; info[ni] = nodeStr; } catch (Exception
					 * e) {}
					 */
				} else
