...
class Scope {
	...
	// check if any plot has the given value (unlike showingValue which checks ALL
	// plots)
	...
	// Trigger edge detection and state machine, called every time the plot ptr
	// advances.
	void checkTrigger() {
	...
	void addValue(int val, CircuitElm ce) {
		if (val == 0) {
			...
			if (ce != null && app.menus.dotsCheckItem.getState() && !(ce instanceof OutputElm
					|| ce instanceof LogicOutputElm || ce instanceof AudioOutputElm || ce instanceof ProbeElm))
	...
	// returns true if we have a plot of voltage and nothing else (except current or
	// charge).
	...
	void clear2dView() {
		if (imageContext != null) {
			if (app.isPrintable()) {
				...
				imageContext.setFillStyle(Theme.scopeBg());
			}
	...
	void draw2d(Graphics g) {
		...
		g.setColor(app.isPrintable() ? Color.white : Theme.scopeBgColor());
		...
		g.context.restore();
		drawSettingsWheel(g);
		if (!app.dialogIsShowing() && rect.contains(app.mouse.mouseCursorX, app.mouse.mouseCursorY)
				&& plots.size() >= 2) {
			double gridPx = calc2dGridPx(rect.width, rect.height);
			...
			double yValue;
			if (isManualScale()) {
				xValue = px.manScale * ((double) (app.mouse.mouseCursorX - rect.x - rect.width / 2) / gridPx
						- manDivisions * px.manVPosition / (double) (V_POSITION_STEPS));
				yValue = py.manScale * ((double) (-app.mouse.mouseCursorY + rect.y + rect.height / 2) / gridPx
				...
				yValue = -((double) (app.mouse.mouseCursorY - rect.y) / (0.499 * (double) (rect.height)) - 1.0)
						* scaleY;
			}
	...
	boolean cursorInSettingsWheel() {
		return showSettingsWheel() && app.mouse.mouseCursorX >= rect.x && app.mouse.mouseCursorX <= rect.x + 36
	...
	void dumpXml(Document doc, Element root) {
		...
		// we don't care about any of these flags because they are all related to old
		// dump format
	...
	void undumpXml(XMLDeserializer xml) {
		...
		// Apply trigger settings from explicit XML attributes (override flag-based
		// values)
