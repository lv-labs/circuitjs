...
public class Toolbar extends FlowPanel {
	...
	public Toolbar() {
		...
		style.setBackgroundColor(Theme.uiSurface());
		style.setBorderWidth(0, Style.Unit.PX);
		style.setProperty("borderBottom", "1px solid " + Theme.uiBorder());
		...
		String gateInfo[] = { andIcon, "AndGateElm", nandIcon, "NandGateElm", orIcon, "OrGateElm", norIcon,
				"NorGateElm", xorIcon, "XorGateElm" };
	...
	private Label createIconButton(String iconClass, String tooltip, MyCommand command) {
		...
		iconLabel.addMouseOverHandler(event -> {
			if (iconLabel != activeButton) {
				iconLabel.getElement().getStyle().setColor(Theme.uiAccent());
				iconLabel.getElement().getStyle().setBackgroundColor(Theme.uiSurface2());
			}
		});
		iconLabel.addMouseOutHandler(event -> {
			if (iconLabel == activeButton) {
				iconLabel.getElement().getStyle().setColor(Theme.uiText());
				iconLabel.getElement().getStyle().setBackgroundColor(Theme.uiSurface2());
			} else {
				iconLabel.getElement().getStyle().setColor(Theme.uiMuted());
				iconLabel.getElement().getStyle().setBackgroundColor(Theme.uiSurface());
			}
		});
	...
	private void setPaletteVisible(FlowPanel palette, boolean visible) {
		palette.setVisible(visible);
		palette.getElement().getStyle().setDisplay(visible ? Style.Display.FLEX : Style.Display.NONE);
		palette.getElement().getStyle().setVisibility(visible ? Style.Visibility.VISIBLE : Style.Visibility.HIDDEN);
		if (!visible) {
			palette.getElement().getStyle().setLeft(-10000, Style.Unit.PX);
			palette.getElement().getStyle().setTop(-10000, Style.Unit.PX);
		}
		if (visible) {
			openPalette = palette;
		} else if (openPalette == palette) {
			openPalette = null;
		}
	}
	...
	private Label createButtonSet(String info[]) {
		...
		setPaletteVisible(paletteContainer, false);
		...
		paletteStyle.setBackgroundColor(Theme.uiSurface2());
		paletteStyle.setBorderWidth(1, Style.Unit.PX);
		paletteStyle.setBorderColor(Theme.uiBorder());
		paletteStyle.setBorderStyle(Style.BorderStyle.SOLID);
		paletteStyle.setPadding(6, Style.Unit.PX);
		paletteStyle.setProperty("display", "flex");
		paletteStyle.setProperty("gap", "4px");
		...
		for (i = 0; i < info.length; i += 2) {
			...
			variantStyle.setColor(Theme.uiMuted());
			variantStyle.setBackgroundColor(Theme.uiSurface2());
			variantStyle.setCursor(Style.Cursor.POINTER);
			variantStyle.setPadding(4, Style.Unit.PX);
			variantStyle.setBorderStyle(Style.BorderStyle.NONE);
			variantStyle.setBorderWidth(0, Style.Unit.PX);

			variantButton.addMouseOverHandler(event -> {
				variantButton.getElement().getStyle().setColor(Theme.uiAccent());
				variantButton.getElement().getStyle().setBackgroundColor(Theme.uiSurface());
			});
			variantButton.addMouseOutHandler(event -> {
				variantButton.getElement().getStyle().setColor(Theme.uiMuted());
				variantButton.getElement().getStyle().setBackgroundColor(Theme.uiSurface2());
			});
		...
		iconLabel.addMouseOverHandler(event -> {
			if (openPalette != null && openPalette != paletteContainer) {
				setPaletteVisible(openPalette, false);
			}
			setPaletteVisible(paletteContainer, true);
		...
		paletteContainer.addDomHandler(event -> setPaletteVisible(paletteContainer, true), MouseOverEvent.getType());
	...
	private void styleModeLabel(Label label) {
		...
		style.setProperty("fontSize", Theme.uiFontSizeSmall());
		style.setProperty("fontWeight", Theme.uiFontWeight());
		style.setColor(Theme.uiMuted());
		style.setMarginLeft(8, Style.Unit.PX);
	...
	public void highlightButton(String key) {
		if (activeButton != null) {
			activeButton.getElement().getStyle().setColor(Theme.uiMuted());
			activeButton.getElement().getStyle().setBackgroundColor(Theme.uiSurface());
		}

		Label newActiveButton = highlightableButtons.get(key);
		if (newActiveButton != null) {
			newActiveButton.getElement().getStyle().setColor(Theme.uiText());
			newActiveButton.getElement().getStyle().setBackgroundColor(Theme.uiSurface2());
	...
	final String resistorIcon = "<svg> <g transform='scale(.5,.5) translate(-544,-297)'>"
			+ "<path stroke='#ffffff' d=' M 544 320 L 552 320' stroke-width='3'/>"
			+ "<path stroke='#ffffff' d=' M 584 320 L 592 320' stroke-width='3'/>"
